                    ...
                    <div class="container-fluid py-5">
                        <div class="row px-xl-5">
                            ...
                            <div class="col-lg-7 pb-5">
                                ...
                                <!-- short description -->
                                <p class="mb-4" style="font-family: roboto; line-height: 1.5; text-align: justify; max-width:600px;"><?= $product['short_description'] ?></p>
                                    ...
                                    <?php if (!empty($sku) && is_array($sku)) : ?>
                                        ...
                                        <?php foreach ($groupedOptions as $variantName => $options) : ?>
                                            <label class="row d-flex " style="max-width:400px;">
                                                ...
                                                <select class="col-5 form-select form-select-lg mb-3 input-select variant-select" data-variant="<?= htmlspecialchars($variantName) ?>" name="variant[<?= htmlspecialchars($variantName) ?>]">
                                                    <?php foreach ($options as $option) : ?>
                                                        <option value="<?= htmlspecialchars($option['id']) ?>"
                                                            data-price="<?= htmlspecialchars($option['price']) ?>"
                                                            data-sku="<?= htmlspecialchars($option['sku']) ?>">
                                ...
                                <script>
                                    ...
                                    function updatePrice() {
                                        let totalPrice = 0; // Không cần giá sản phẩm, chỉ tính từ SKU
                                        document.querySelectorAll('.variant-select').forEach(select => {
                                            let selectedOption = select.options[select.selectedIndex];
                                            let optionPrice = parseFloat(selectedOption.getAttribute('data-price'));
                                            totalPrice += optionPrice;
                                        });
                                        document.getElementById('price-display').innerHTML = `<strong class="text-danger">${numberFormat(totalPrice)}đ</strong>`;
                                    }
                                    ...
                                    function updateSelectedVariants() {
                                        ...
                                        document.querySelectorAll('.variant-select').forEach(select => {
                                            let variantName = select.getAttribute('data-variant');
                                            let selectedOption = select.options[select.selectedIndex];
                                    ...
                                    // Gọi hàm updateSelectedVariants khi trang được tải
                                    updateSelectedVariants();
                                    updatePrice();
                                </script>


                                <div class="card-footer d-flex bg-light align-items-center" style="margin-top:150px;">
                                    <form action="/cart/add" method="post" class="d-flex align-items-center">
                                        ...
                                        <button type="submit" class="btn btn-primary px-1 add-to-cart-btn" style="width: 280px;">
                    ...
                    <div class="row px-xl-5">
                        <div class="col border-bottom pb-4">
                            <div class="nav nav-tabs justify-content-center border-secondary mb-4">
                                ...
                                <a class="nav-item nav-link " data-toggle="tab" href="#tab-pane-2">Hướng dẫn sử dụng</a>
